...
class Shop extends Model
{
            ...
            switch ($key) {
                ...
                case 'class':
                    if (!isset($content['traffic_package'])){
                        $content_text .= ', 为账号升级为等级 ' . $value . ' , 有效期 ' . isset($content['class_expire']) ? $content['class_expire'] : 0 . ' 天 ';
                    }
                    break;
                case 'reset':
    ...
    {
        $content = json_decode($this->attributes['content']);
        return isset($content->class) ? $content->class : 0;
    }

    public function class_expire()
    {
        $content = json_decode($this->attributes['content']);
        return isset($content->class_expire) ? $content->class_expire : 0;
    }

    public function speedlimit()
    {
        $content = json_decode($this->attributes['content']);
        return isset($content->speedlimit) ? $content->speedlimit : 0;
    }

    public function connector()
    {
        $content = json_decode($this->attributes['content']);
        return isset($content->connector) ? $content->connector : 0;
    }

    public function node_group()
    {
        $content = json_decode($this->attributes['content']);
        return isset($content->node_group) ? $content->node_group : -1;
    }
                ...
                case 'bandwidth':
                    if ($is_renew == 0) {
                        if ($_ENV['enable_bought_reset'] == true) {
                            ...
                            $user->d = 0;
                            $user->last_day_t = 0;
                        } elseif((int)$user->class != (int)$content['class']) {
                            $user->transfer_enable = $value * 1024 * 1024 * 1024;
                            $user->u = 0;
                            $user->d = 0;
                            $user->last_day_t = 0;
